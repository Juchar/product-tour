...
class Step extends Evented {
  ...
  bindAdvance() {
    ...
    getVaadinOverlayContainer().addEventListener(event, handler);
    this.on('destroy', () => {
      return getVaadinOverlayContainer().removeEventListener(event, handler);
    });
  }
  ...
  show(fireEvents = true) {
    if (!isUndefined(this.options.beforeShowPromise)) {
      const beforeShowPromise = this.options.beforeShowPromise();
      if (!isUndefined(beforeShowPromise)) {
        return beforeShowPromise.then(() => this._show(fireEvents));
      }
    }
    this._show(fireEvents);
  }
  ...
  render() {
    ...
    this.el =
        createFromHTML(`<div class='shepherd-step ${ this.options.classes
                                                     || '' }' data-id='${ this.id }' ${ this.options.idAttribute
            ? 'id="' + this.options.idAttribute + '"' : '' }></div>`);

    let content = document.createElement('div');
    content.className = 'shepherd-content';
    if (this.options.width != null) {
      content.style.width = this.options.width;
    }
    ...
    if (this.options.title) {
      header.innerHTML += `<span class='shepherd-title'>${ this.options.title }</span>`;
    ...
    if (this.options.buttons) {
      ...
      this.options.buttons.map(cfg => {
        const button = createFromHTML(
            `<li><a class='shepherd-button ${ cfg.classes || '' } ${ cfg.buttonEnabled ? ''
                : 'disabled'}'>${ cfg.text }</a>`);
        buttons.appendChild(button);
        if (cfg.buttonEnabled) {
    ...
    getVaadinOverlayContainer().appendChild(this.el);
...
class Tour extends Evented {
  ...
  show(key = 0, forward = true, fireEvents = true) {
    ...
    if (next) {
      if (!isUndefined(next.options.showOn) && !next.options.showOn()) {
        ...
        this.show(nextIndex, forward, fireEvents);
      } else {
        if (fireEvents) {
          this.trigger('show', {
            step: next,
